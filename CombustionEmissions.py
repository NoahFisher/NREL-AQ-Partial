...
#                print "Current file is: %s -- %s" % (run_code, cur_file)
                ...
                # write data to static files and the database
                        ...
                        cur.execute("""INSERT INTO %s.%s_raw (FIPS, SCC, HP, THC, VOC, CO, NOX, CO2, SO2, PM10, PM25, fuel_consumption, NH3, description, run_code) 
...
# Switchgrass        
        ...
        elif run_code.startswith('SG_NH'):
            if len(run_code) == 4: 
                description = "Year %s - Non-Harvest" % (run_code[5])  # year 1-9
            else:
                description = "Year %s - Non-Harvest" % (run_code[5:7])  # year 10
